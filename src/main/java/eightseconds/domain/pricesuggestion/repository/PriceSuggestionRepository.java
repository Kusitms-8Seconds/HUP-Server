...
public interface PriceSuggestionRepository extends JpaRepository<PriceSuggestion, Long> {
    ...
    @Query("SELECT p FROM PriceSuggestion p WHERE p.suggestionPrice = (SELECT max(subP.suggestionPrice) FROM PriceSuggestion subP)")
    Optional<PriceSuggestion> getMaximumPriceByItemId(Long itemId);
